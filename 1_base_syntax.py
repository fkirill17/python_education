...
if i := d.get(''):  # Выражение в условии преобразуется в булевы тип -> bool(i := d.get(''))
    pass
        ...
        1)):  # Дополнительное условие, проверяется если if -> False (Необязательный блок, их количество не ограничено)
    # not - логическая инверсия
    pass
elif ...:  # Условие проверяется, если предыдущий elif -> False
    pass
...
class MyExcept(Exception):  # Создание пользовательской ошибки
    pass


try:  # Блок кода, в котором ожидаем исключение
    raise MyExcept  # Явное возбуждение ошибки, сразу перемещаемся в блок except ZeroDivisionError
except MyExcept as e:  # Отлавливаем ошибку, допускается несколько блоков except подряд. Excception - отловит все ошибки
    pass
else:  # Код, который выполняется, если в блоке try не возникло ошибки (Необязательный блок)
    pass
finally:  # Код, гарантированно выполняется даже если возникла ошибка (Необязательный блок)
    pass
# О блоке finally:
# Гарантированное выполнение: finally выполняется даже при return, break, sys.exit().
# Используется для очистки: Закрытие файлов, соединений с БД, освобождение памяти и т. д.
