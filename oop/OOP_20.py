...
class Vector:
    x = IntDescriptor()  # Дескриптор-1 Внутри создаётся словарь с ключем _values и при вызове __set__ делается запись
    y = IntDescriptor()  # Дескриптор-2
...
print(list(Vector.x._values.keys())[0])  # Удаление обхъекта не произошло
